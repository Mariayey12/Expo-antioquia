...
namespace Database\Seeders;
    ...
    //
        ...
        // Crear el usuario relacionado con el proveedor (relación polimórfica)
        $provider->user()->create([
            'name' => $faker->name, // Nombre aleatorio generado por Faker
            'email' => $faker->unique()->safeEmail, // Correo electrónico único
            'email_verified_at' => now(), // Fecha actual como verificación del correo
            'company_name' => $faker->company, // Nombre de empresa generado por Faker
            'password' => bcrypt('password'), // Contraseña encriptada
            'contact_person' => $faker->name, // Persona de contacto generada por Faker
            'phone' => $faker->phoneNumber, // Teléfono generado por Faker
            'address' => $faker->address, // Dirección generada por Faker
            'profile_picture' => $faker->imageUrl(200, 200), // URL de imagen generada por Faker
            'role' => 'usuario', // Rol predefinido (puedes cambiarlo según el caso)
            'remember_token' => Str::random(10), // Token aleatorio para recordar la sesión
            'created_at' => now(), // Fecha de creación
            'updated_at' => now(), // Fecha de actualización
            'userable_type' => get_class($provider), // Tipo del modelo (Provider)
